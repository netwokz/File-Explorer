...
public class Presenter implements LoaderManager.LoaderCallbacks<List<File>> {
    ...
    private AsyncTaskLoader<List<File>> mFileLoader; /*Loads the list of files from the model in a background thread.*/
    ...
    private void updateAdapter(List<File> data) {
        ...
        //inform the ListView to refresh itself with the new data.
        mFileArrayAdapter.notifyDataSetChanged();

        TextView pathDetails = (TextView) mView.getActivity().findViewById(R.id.tv_path2);
        pathDetails.setText(getDir());
    }
    ...
    public void settings() {
        Toast.makeText(mView.getActivity(), "settings clicked", Toast.LENGTH_LONG).show();
    }
    ...
    public String getDir() {
        return mModel.getmCurrentDir().getAbsolutePath();
    }
}